...
def do_stuff(input_text, number_of_sentences):
    ...
    freqDist = nltk.FreqDist(tokenize(cleaned_text))
    sentence_list = sentencize(cleaned_text)
    #print(sentence_list)
    sentence_value_dict = {}
    for sentence in sentence_list:
        value = 0;
        stopword_cleaned_sentence = remove_words(sentence, remove_list)
        tokens = tokenize(stopword_cleaned_sentence)
        #print(tokens)
        for token in tokens:
            value += freqDist[token]
...
def sentencize(input_text):
    ...
    if use_nltk_tokenization:
        ...
        return re.findall(r".+[.!?]+ ", input_text) # TODO (doesn't work correctly)
